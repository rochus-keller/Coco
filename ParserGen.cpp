...
// NOTE: modified by Rochus, Expect and SynErr extendet by __FUNCTION__ argument (works also with VC2013 in contrast to __func__)
namespace Coco {
	...
	while (p != NULL) {
		if (p->typ == Node::nt) {
			...
			if (tab->terminals->Count == (acc + 1) || (acc > 0 && Sets::Equals(p->set, isChecked))) {
				...
				if (acc > 0) {
					fwprintf(gen, L"if ("); GenCond(p->set, p); fwprintf(gen, L") Get(); else SynErr(%d,__FUNCTION__);\n", errorNr);
				} else fwprintf(gen, L"SynErr(%d,__FUNCTION__); // ANY node that matches no symbol\n", errorNr);
			}
			...
			fwprintf(gen, L"SynErr(%d,__FUNCTION__); Get();", errorNr); fwprintf(gen, L"}\n");
		} if (p->typ == Node::alt) {
			s1 = tab->First(p);
			...
			if (equal) {
				...
				GenErrorMsg(altErr, curSy);
				if (useSwitch) {
					fwprintf(gen, L"default: SynErr(%d,__FUNCTION__); break;\n", errorNr);
					Indent(indent); fwprintf(gen, L"}\n");
				} else {
					fwprintf(gen, L"} "); fwprintf(gen, L"else SynErr(%d,__FUNCTION__);\n", errorNr);
				}
			}
		} if (p->typ == Node::iter) {
			Indent(indent);
...
void ParserGen::WriteParser () {
	...
	g.CopyFramePart(L"-->parseRoot"); fwprintf(gen, L"\t%ls();\n", tab->gramSy->name); if (tab->checkEOF) fwprintf(gen, L"\tExpect(0,__FUNCTION__);");
