...
void Parser::A() {
		if (false) {
			SynErr(11,__FUNCTION__); // ANY node that matches no symbol
		} else if (StartOf(1)) {
...
void Parser::B() {
		if (false) {
			SynErr(13,__FUNCTION__); // ANY node that matches no symbol
		} else if (StartOf(1)) {
			Get();
...
void Parser::C() {
		while (false) {
			SynErr(15,__FUNCTION__); // ANY node that matches no symbol
		}
		Get();
}
...
void Parser::D() {
		if (false) {
			SynErr(16,__FUNCTION__); // ANY node that matches no symbol
		}
		Get();
}
...
void Errors::SynErr(int line, int col, int n) {
	...
	switch (n) {
			...
			case 12: s = coco_string_create(L"invalid A"); break;
			case 13: s = coco_string_create(L"invalid B"); break;
			case 14: s = coco_string_create(L"invalid B"); break;
			case 15: s = coco_string_create(L"invalid C"); break;
			case 16: s = coco_string_create(L"invalid D"); break;
