...
void Parser::Test() {
		...
		while (!(StartOf(1))) {SynErr(11,__FUNCTION__); Get();}
...
void Parser::A() {
		while (!(la->kind == _EOF || la->kind == _e || la->kind == _g)) {SynErr(12,__FUNCTION__); Get();}
		if (la->kind == _e) {
			Get();
			Expect(_f,__FUNCTION__);
		}
...
void Errors::SynErr(int line, int col, int n) {
	...
	switch (n) {
			...
			case 11: s = coco_string_create(L"this symbol not expected in Test"); break;
			case 12: s = coco_string_create(L"this symbol not expected in A"); break;
