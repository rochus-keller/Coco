...
Token* Scanner::NextToken() {
	...
	switch (state) {
		...
		case 9:
			recEnd = pos; recKind = 1;
			if ((ch >= L'0' && ch <= L'9') || (ch >= L'A' && ch <= L'Z') || (ch >= L'a' && ch <= L'z')) {apx = 0; AddCh(); goto case_1;}
			else if (ch == L'*') {apx = 0; AddCh(); goto case_2;}
			else if (ch == L'_') {apx++; AddCh(); goto case_11;}
		...
		case 10:
			...
			else if (ch == L'.') {apx++; AddCh(); goto case_12;}
			else {t->kind = 4; break;}
		case 11:
			case_11:
			if (ch == L'*') {apx = 0; AddCh(); goto case_2;}
			else if (ch == L'_') {apx++; AddCh(); goto case_11;}
			else if (ch == L'+') {apx++; AddCh(); goto case_3;}
			else {goto case_0;}
		case 12:
			...
			else if (ch == L'E') {apx = 0; AddCh(); goto case_5;}
			else if (ch == L'.') {apx++; AddCh(); goto case_8;}
			else {t->kind = 4; break;}
		case 13:
			case_13:
			{t->kind = 8; break;}
		...
		case 15:
			recEnd = pos; recKind = 1;
			if ((ch >= L'0' && ch <= L'9') || (ch >= L'A' && ch <= L'Z') || ch == L'a' || (ch >= L'c' && ch <= L'z')) {apx = 0; AddCh(); goto case_1;}
			else if (ch == L'*') {apx = 0; AddCh(); goto case_2;}
			else if (ch == L'_') {apx++; AddCh(); goto case_16;}
			else if (ch == L'+') {apx++; AddCh(); goto case_3;}
			else if (ch == L'b') {apx = 0; AddCh(); goto case_17;}
			else {t->kind = 1; wchar_t *literal = coco_string_create(tval, 0, tlen); t->kind = keywords.get(literal, t->kind); coco_string_delete(literal); break;}
		case 16:
			case_16:
			recEnd = pos; recKind = 10;
			if (ch == L'*') {apx = 0; AddCh(); goto case_2;}
			else if (ch == L'_') {apx++; AddCh(); goto case_18;}
			else if (ch == L'+') {apx++; AddCh(); goto case_3;}
			else {t->kind = 10; break;}
		case 17:
			case_17:
			recEnd = pos; recKind = 1;
			if ((ch >= L'0' && ch <= L'9') || (ch >= L'A' && ch <= L'Z') || (ch >= L'a' && ch <= L'b') || (ch >= L'd' && ch <= L'z')) {AddCh(); goto case_1;}
			else if (ch == L'c') {AddCh(); goto case_19;}
			else {t->kind = 1; wchar_t *literal = coco_string_create(tval, 0, tlen); t->kind = keywords.get(literal, t->kind); coco_string_delete(literal); break;}
		case 18:
			case_18:
			if (ch == L'*') {apx = 0; AddCh(); goto case_20;}
			else if (ch == L'_') {apx++; AddCh(); goto case_11;}
			else if (ch == L'+') {apx++; AddCh(); goto case_3;}
			else {goto case_0;}
		case 19:
